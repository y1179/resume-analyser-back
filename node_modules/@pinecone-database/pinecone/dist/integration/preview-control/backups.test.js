...
describe('preview backups', () => {
    ...
    beforeAll(async () => {
        ...
        const indexName = (0, test_helpers_1.randomName)('preview-backup-src');
        index = (await pc.createIndex({
            ...
            waitUntilReady: true,
        })); // we're not suppressing conflicts so we know the response should be an IndexModel
    ...
    describe('preview listProjectBackups', () => {
        test('returns a list including existing backup', async () => {
            ...
            expect(result.data.length).toBeGreaterThanOrEqual(1);
            const someBackup = result.data.some((currBackup) => currBackup.backupId === backup.backupId);
    ...
    describe('preview listRestoreJobs/describeRestoreJob', () => {
        test('returns RestoreJobModel for an existing restore job if one exists', async () => {
            ...
            if (result.data.length > 0) {
                const described = await pc.preview.indexes.describeRestoreJob(result.data[0].restoreJobId);
        ...
        test('throws PineconeNotFoundError for a non-existent jobId', async () => {
            await expect(pc.preview.indexes.describeRestoreJob('nonexistent-job-id-xyz')).rejects.toThrow(errors_1.PineconeNotFoundError);
        });
    });
});
