...
const buildFetchMock = () => {
    ...
    const mockFetch = jest.fn(async (input, init) => {
        ...
        throw new Error(`Unexpected request in mocked smoke test: ${method} ${url}`);
    });
    return { mockFetch, hits };
};
const countHits = (hits, method, url) => hits.filter((h) => h.method === method && h.url === url).length;
describe('mocked critical path (no API key)', () => {
    ...
    test('connect → upsert → query works end to end against a mocked backend', async () => {
        ...
        expect(countHits(hits, 'GET', `${CONTROL_PLANE}/indexes/${INDEX_NAME}`)).toBe(1);
...
//# sourceMappingURL=mockedCriticalPath.test.js.map