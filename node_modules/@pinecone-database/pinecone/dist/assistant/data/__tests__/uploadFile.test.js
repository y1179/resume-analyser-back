...
describe('path input', () => {
    ...
    test('sends metadata in form body', async () => {
        ...
        expect(mockRetryingFetch).toHaveBeenCalledWith('https://prod-1-data.ke.pinecone.io/assistant/files/test-assistant', expect.objectContaining({ body: expect.any(FormData) }));
        const body = mockRetryingFetch.mock.calls[0][1].body;
        const metadataBlob = body.get('metadata');
    ...
    test('sends metadata in form body with multimodal query param', async () => {
        ...
        expect(mockRetryingFetch).toHaveBeenCalledWith('https://prod-1-data.ke.pinecone.io/assistant/files/test-assistant?multimodal=true', expect.objectContaining({ body: expect.any(FormData) }));
        const body = mockRetryingFetch.mock.calls[0][1].body;
        const metadataBlob = body.get('metadata');
        expect(await metadataBlob.text()).toBe(JSON.stringify(metadata));
    });
...
describe('ReadableStream input', () => {
    ...
    test('sends metadata in multipart body (not URL)', async () => {
        const upload = (0, uploadFile_1.uploadFile)(mockAssistantName, mockApiProvider, mockConfig);
        const stream = stream_1.Readable.from([Buffer.from('data')]);
        ...
        expect(url).toBe('https://prod-1-data.ke.pinecone.io/assistant/files/test-assistant');
