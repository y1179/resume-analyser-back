...
describe('deleteMany', () => {
    ...
    test('throws if pass no record IDs', async () => {
        ...
        await expect(toThrow()).rejects.toThrow(errors_1.PineconeArgumentError);
        await expect(toThrow()).rejects.toThrow('Must pass in at least 1 record ID.');
    });
    test('throws if both ids and filter are provided', async () => {
        ...
        await expect(toThrow()).rejects.toThrow(errors_1.PineconeArgumentError);
        await expect(toThrow()).rejects.toThrow('Cannot provide both `ids` and `filter`');
    });
    test('throws if neither ids nor filter are provided', async () => {
        ...
        await expect(toThrow()).rejects.toThrow(errors_1.PineconeArgumentError);
        await expect(toThrow()).rejects.toThrow('Either `ids` or `filter` must be provided');
    });
