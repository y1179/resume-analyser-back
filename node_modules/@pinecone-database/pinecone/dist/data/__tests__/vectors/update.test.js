...
describe('update', () => {
    ...
    test('throws error if no id or filter are provided', async () => {
        ...
        await expect(toThrow()).rejects.toThrow('You must pass a non-empty string for the `id` field or a `filter` object in order to update records.');
    });
    test('throws error if both id and filter are provided', async () => {
        ...
        await expect(toThrow()).rejects.toThrow('You cannot pass both an `id` and a `filter` object to update records. Use either `id` to update a single record, or `filter` to update multiple records.');
    });
