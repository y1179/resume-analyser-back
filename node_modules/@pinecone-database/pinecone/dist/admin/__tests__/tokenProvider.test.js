"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const tokenProvider_1 = require("../tokenProvider");
const errors_1 = require("../../errors");
const makeResponse = (body, { ok = true, status = 200 } = {}) => ({
    ok,
    status,
    json: async () => (typeof body === 'string' ? JSON.parse(body) : body),
    text: async () => (typeof body === 'string' ? body : JSON.stringify(body)),
});
const tokenBody = (accessToken, expiresIn = 1800) => ({
    access_token: accessToken,
    token_type: 'Bearer',
    expires_in: expiresIn,
});
describe('TokenProvider', () => {
    afterEach(() => {
        jest.restoreAllMocks();
    });
    test('exchanges client credentials at the OAuth endpoint with the expected payload', async () => {
        const fetchMock = jest
            .fn()
            .mockResolvedValue(makeResponse(tokenBody('tok-abc')));
        const provider = new tokenProvider_1.TokenProvider('my-client-id', 'my-client-secret', fetchMock, 'test-user-agent');
        const token = await provider.getToken();
        expect(token).toBe('tok-abc');
        expect(fetchMock).toHaveBeenCalledTimes(1);
        const [url, init] = fetchMock.mock.calls[0];
        expect(url).toBe(tokenProvider_1.OAUTH_TOKEN_URL);
        expect(init?.method).toBe('POST');
        expect((init?.headers)['Content-Type']).toBe('application/json');
        expect(JSON.parse(init?.body)).toEqual({
            client_id: 'my-client-id',
            client_secret: 'my-client-secret',
            grant_type: 'client_credentials',
            audience: tokenProvider_1.OAUTH_AUDIENCE,
        });
    });
    test('caches the token and does not re-fetch while it is still valid', async () => {
        const fetchMock = jest
            .fn()
            .mockResolvedValue(makeResponse(tokenBody('tok-cached')));
        const provider = new tokenProvider_1.TokenProvider('id', 'secret', fetchMock, 'ua');
        const first = await provider.getToken();
        const second = await provider.getToken();
        expect(first).toBe('tok-cached');
        expect(second).toBe('tok-cached');
        expect(fetchMock).toHaveBeenCalledTimes(1);
    });
    test('caches for the lifetime of the client and never proactively refreshes', async () => {
        // A second token is queued to prove it is never requested; one exchange should serve every call.
        const fetchMock = jest
            .fn()
            .mockResolvedValueOnce(makeResponse(tokenBody('tok-1', 1800)))
            .mockResolvedValueOnce(makeResponse(tokenBody('tok-2', 1800)));
        const provider = new tokenProvider_1.TokenProvider('id', 'secret', fetchMock, 'ua');
        for (let i = 0; i < 5; i++) {
            expect(await provider.getToken()).toBe('tok-1');
        }
        expect(fetchMock).toHaveBeenCalledTimes(1);
    });
    test('coalesces concurrent refreshes into a single request (single-flight)', async () => {
        let resolveFetch = () => { };
        const fetchMock = jest.fn().mockImplementation(() => new Promise((resolve) => {
            resolveFetch = resolve;
        }));
        const provider = new tokenProvider_1.TokenProvider('id', 'secret', fetchMock, 'ua');
        const p1 = provider.getToken();
        const p2 = provider.getToken();
        resolveFetch(makeResponse(tokenBody('tok-shared')));
        expect(await p1).toBe('tok-shared');
        expect(await p2).toBe('tok-shared');
        expect(fetchMock).toHaveBeenCalledTimes(1);
    });
    test('throws PineconeAuthorizationError when credentials are rejected (401)', async () => {
        const fetchMock = jest
            .fn()
            .mockResolvedValue(makeResponse({ error: 'access_denied', error_description: 'bad creds' }, { ok: false, status: 401 }));
        const provider = new tokenProvider_1.TokenProvider('id', 'secret', fetchMock, 'ua');
        await expect(provider.getToken()).rejects.toThrow(errors_1.PineconeAuthorizationError);
        // The message must describe the OAuth credentials, not an Api-Key, and surface the server detail.
        await expect(provider.getToken()).rejects.toThrow(/clientId and clientSecret/);
        await expect(provider.getToken()).rejects.toThrow(/bad creds/);
    });
    test('throws PineconeUnexpectedResponseError on other non-2xx statuses', async () => {
        const fetchMock = jest
            .fn()
            .mockResolvedValue(makeResponse({ error: 'server_error', error_description: 'boom' }, { ok: false, status: 500 }));
        const provider = new tokenProvider_1.TokenProvider('id', 'secret', fetchMock, 'ua');
        await expect(provider.getToken()).rejects.toThrow(errors_1.PineconeUnexpectedResponseError);
    });
    test('throws when the response is missing an access_token', async () => {
        const fetchMock = jest
            .fn()
            .mockResolvedValue(makeResponse({ token_type: 'Bearer', expires_in: 1800 }));
        const provider = new tokenProvider_1.TokenProvider('id', 'secret', fetchMock, 'ua');
        await expect(provider.getToken()).rejects.toThrow(errors_1.PineconeUnexpectedResponseError);
    });
    test('a failed exchange clears the in-flight guard so a later call can retry', async () => {
        const fetchMock = jest
            .fn()
            .mockResolvedValueOnce(makeResponse({ error: 'server_error' }, { ok: false, status: 500 }))
            .mockResolvedValueOnce(makeResponse(tokenBody('tok-recovered')));
        const provider = new tokenProvider_1.TokenProvider('id', 'secret', fetchMock, 'ua');
        await expect(provider.getToken()).rejects.toThrow();
        expect(await provider.getToken()).toBe('tok-recovered');
        expect(fetchMock).toHaveBeenCalledTimes(2);
    });
});
//# sourceMappingURL=tokenProvider.test.js.map