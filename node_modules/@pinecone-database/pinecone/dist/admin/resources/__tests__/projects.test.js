...
const setup = () => {
    ...
    return { api, projects: new projects_1.ProjectsResource(api) };
};
describe('ProjectsResource', () => {
    ...
    test('create throws when name is missing', async () => {
        ...
        await expect(projects.create({})).rejects.toThrow(errors_1.PineconeArgumentError);
    });
    ...
    test('describe throws when projectId is empty', async () => {
        const { projects } = setup();
        await expect(projects.describe('')).rejects.toThrow(errors_1.PineconeArgumentError);
    });
    ...
    test('update throws when projectId is empty', async () => {
        const { projects } = setup();
        await expect(projects.update('', { name: 'x' })).rejects.toThrow(errors_1.PineconeArgumentError);
    });
    ...
    test('delete throws when projectId is empty', async () => {
        const { projects } = setup();
        await expect(projects.delete('')).rejects.toThrow(errors_1.PineconeArgumentError);
    });
});
