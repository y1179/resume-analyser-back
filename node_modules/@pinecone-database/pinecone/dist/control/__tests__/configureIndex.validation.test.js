...
describe('configureIndex argument validations', () => {
    ...
    describe('required configurations', () => {
        test('should throw if index name is not provided', async () => {
            ...
            await expect(toThrow).rejects.toThrow('You must pass a non-empty string for `name` to configureIndex.');
        });
        ...
        test('should throw if spec or deletionProtection are not provided', async () => {
            ...
            await expect(toThrowSpec).rejects.toThrow(errors_1.PineconeArgumentError);
            await expect(toThrowSpec).rejects.toThrow('You must pass at least one configuration option to configureIndex.');
        });
        test('should throw if no configuration options are provided', async () => {
            ...
            await expect(toThrow).rejects.toThrow(errors_1.PineconeArgumentError);
            await expect(toThrow).rejects.toThrow('You must pass at least one configuration option to configureIndex.');
        });
    });
});
