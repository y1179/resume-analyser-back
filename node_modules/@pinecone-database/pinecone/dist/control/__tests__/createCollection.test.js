...
describe('createCollection', () => {
    describe('argument validations', () => {
        test('throws if no arguments are provided', async () => {
            ...
            await expect(toThrow).rejects.toThrow('You must pass a non-empty object with `name` and `source` fields in order to create a collection.');
        });
        test('throws if empty object', async () => {
            ...
            await expect(toThrow).rejects.toThrow(errors_1.PineconeArgumentError);
            await expect(toThrow).rejects.toThrow('The argument to createCollection must have required properties: `name`, `source`.');
        });
        test('throws if name is not provided', async () => {
            ...
            await expect(toThrow).rejects.toThrow(errors_1.PineconeArgumentError);
            await expect(toThrow).rejects.toThrow('You must pass a non-empty string for `name` in order to create a collection.');
        });
        test('throws if source is not provided', async () => {
            ...
            await expect(toThrow).rejects.toThrow(errors_1.PineconeArgumentError);
            await expect(toThrow).rejects.toThrow('You must pass a non-empty string for `source` in order to create a collection.');
        });
