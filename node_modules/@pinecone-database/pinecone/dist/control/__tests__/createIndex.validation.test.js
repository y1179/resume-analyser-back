...
describe('createIndex argument validations', () => {
    ...
    describe('required configurations', () => {
        test('should throw no options are provided', async () => {
            ...
            await expect(toThrow).rejects.toThrow(errors_1.PineconeArgumentError);
            await expect(toThrow).rejects.toThrow('You must pass an object with required properties (`name`, `dimension`, `spec`) to create an index.');
        });
        ...
        test('should throw if dimension is not a positive integer', async () => {
            ...
            await expect(toThrow).rejects.toThrow(errors_1.PineconeArgumentError);
            await expect(toThrow).rejects.toThrow('You must pass a positive integer for `dimension` in order to create an index.');
        });
        test('should throw if region is not provided', () => {
            ...
            expect(toThrow).rejects.toThrow(errors_1.PineconeArgumentError);
            expect(toThrow).rejects.toThrow('You must pass a `region` for the serverless `spec` object in order to create an index.');
        });
        test('should throw if cloud is not provided', () => {
            ...
            expect(toThrow).rejects.toThrow(errors_1.PineconeArgumentError);
            expect(toThrow).rejects.toThrow('You must pass a `cloud` for the serverless `spec` object in order to create an index.');
        });
    ...
    describe('optional configurations', () => {
        test('metric: should throw if not one of the predefined literals', async () => {
            ...
            await expect(toThrow).rejects.toThrow(errors_1.PineconeArgumentError);
            await expect(toThrow).rejects.toThrow("Invalid metric value: foo. Valid values are: 'cosine', 'euclidean', or 'dotproduct.'");
        });
        test('replicas: should throw if not a positive integer', async () => {
            ...
            await expect(toThrow).rejects.toThrow(errors_1.PineconeArgumentError);
            await expect(toThrow).rejects.toThrow('You must pass a positive integer for `replicas` in order to create an index.');
        });
        test('podType: should throw if not a valid pod type', async () => {
            ...
            await expect(toThrow).rejects.toThrow(errors_1.PineconeArgumentError);
            await expect(toThrow).rejects.toThrow('Invalid pod type: gobblygook. Valid values are: s1.x1, s1.x2, s1.x4, s1.x8, p1.x1, p1.x2, p1.x4, p1.x8, p2.x1, p2.x2, p2.x4, p2.x8.');
        });
        test('pods: should throw if not a positive integer', async () => {
            ...
            await expect(toThrow).rejects.toThrow(errors_1.PineconeArgumentError);
            await expect(toThrow).rejects.toThrow('You must pass a positive integer for `pods` in order to create an index.');
        });
    });
});
