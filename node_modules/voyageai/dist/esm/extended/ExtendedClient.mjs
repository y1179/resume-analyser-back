...
import { VoyageAIClient as GeneratedClient } from "../Client.mjs";
import { HttpResponsePromise } from "../core/fetcher/HttpResponsePromise.mjs";
import { unknownRawResponse } from "../core/fetcher/RawResponse.mjs";
import { isLocalModel, localEmbed } from "../local/index.mjs";
import { tokenizeTexts } from "../local/tokenizer.mjs";
export class VoyageAIClient extends GeneratedClient {
    ...
    embed(request, requestOptions) {
        if (isLocalModel(request.model)) {
            return HttpResponsePromise.fromPromise(localEmbed(request).then((data) => ({ data, rawResponse: unknownRawResponse })));
        }
    ...
    contextualizedEmbed(request, requestOptions) {
        const _super = Object.create(null, {
            contextualizedEmbed: { get: () => super.contextualizedEmbed }
        });
        return __awaiter(this, void 0, void 0, function* () {
            const { chunkFn } = request, apiRequest = __rest(request, ["chunkFn"]);
            const normalizedRequest = validateAndNormalizeContextualizedInputs(apiRequest, chunkFn);
            const requestInputs = chunkFn
                ? applyChunking(normalizedRequest.inputs, chunkFn)
                : normalizedRequest.inputs;
            const response = yield _super.contextualizedEmbed.call(this, Object.assign(Object.assign({}, normalizedRequest), { inputs: requestInputs }), requestOptions);
            const clientChunkTexts = chunkFn ? requestInputs : undefined;
     ...
     */
    tokenize(texts, model) {
        return __awaiter(this, void 0, void 0, function* () {
            return tokenizeTexts(model, texts);
        });
    }
}
export function buildContextualizedResult(response, clientChunkTexts) {
    var _a, _b, _c, _d, _e;
    const results = [];
    const serverTextsPerDoc = [];
    for (let i = 0; i < ((_a = response.data) !== null && _a !== void 0 ? _a : []).length; i++) {
        const doc = response.data[i];
        const embeddings = ((_b = doc.data) !== null && _b !== void 0 ? _b : []).map((chunk) => { var _a; return (_a = chunk.embedding) !== null && _a !== void 0 ? _a : []; });
        let chunkTexts;
        if (clientChunkTexts !== undefined) {
            chunkTexts = clientChunkTexts[i];
        }
        else {
            const perDocTexts = ((_c = doc.data) !== null && _c !== void 0 ? _c : []).map((chunk) => chunk.text);
            if (perDocTexts.every((t) => t !== undefined)) {
                serverTextsPerDoc.push(perDocTexts);
                chunkTexts = perDocTexts;
            }
            else if (perDocTexts.some((t) => t !== undefined)) {
                throw new Error(`inputs[${i}] returned a partial set of chunk texts; expected text on every chunk or none`);
            }
            else {
    ...
    let resultChunkTexts;
    if (clientChunkTexts !== undefined) {
        resultChunkTexts = clientChunkTexts;
    }
    else if (serverTextsPerDoc.length > 0) {
        const populated = serverTextsPerDoc.filter((t) => t !== undefined);
    ...
    return {
        results,
        totalTokens: (_e = (_d = response.usage) === null || _d === void 0 ? void 0 : _d.totalTokens) !== null && _e !== void 0 ? _e : 0,
        chunkTexts: resultChunkTexts,
        chunkerVersion: response.chunkerVersion,
        rawResponse: response,
    };
}
function isFlatStringArray(inputs) {
    return inputs.length > 0 && inputs.every((item) => typeof item === "string");
}
export function validateAndNormalizeContextualizedInputs(request, chunkFn) {
    ...
    if (enableAutoChunking) {
        ...
        for (let i = 0; i < normalizedInputs.length; i++) {
            if (normalizedInputs[i].length !== 1) {
                throw new Error(`inputs[${i}] has ${normalizedInputs[i].length} chunks; auto-chunking expects one string per document`);
            }
        }
    }
    return Object.assign(Object.assign({}, request), { inputs: normalizedInputs });
}
export function applyChunking(inputs, chunkFn) {
...
export function defaultChunkFn(chunkSize = DEFAULT_CHUNK_SIZE, chunkOverlap = 0) {
    return (text) => {
        if (text.length === 0) {
            return [text];
        }
